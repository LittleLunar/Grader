...
def check_binary_search_tree_(root, _min=0, _max=100):
    if not root:
        return True
    if root.data < _min or root.data > _max:
        return False
    return check_binary_search_tree_(root.left, _min, root.data - 1) and check_binary_search_tree_(root.right, root.data + 1, _max)
